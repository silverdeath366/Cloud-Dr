#!/usr/bin/env python3
"""
Service A - CloudPhoenix Application Service
...
def cloud_status():
    """Get current cloud provider and status"""
    try:
        # Detect cloud provider from environment variables or metadata
        cloud_provider = os.getenv('CLOUD_PROVIDER', 'aws')
        
        # Try to detect from metadata services
        try:
            # AWS metadata
        ...
        db_host = os.getenv('DB_HOST', '')
        db_type = 'Unknown'
        if '.rds.amazonaws.com' in db_host or 'rds' in db_host.lower():
            db_type = 'RDS PostgreSQL'
            cloud_provider = 'aws'
        elif 'database.windows.net' in db_host or 'azure' in db_host.lower():
            db_type = 'Azure SQL'
            cloud_provider = 'azure'
        ...
        return jsonify({
            ...
            'db_type': db_type,
            'db_status': db_status,
            'region': os.getenv('AWS_REGION') or os.getenv('AZURE_LOCATION', 'unknown'),
            'timestamp': datetime.utcnow().isoformat(),
            'request_id': g.request_id
        }), 200
    except Exception as e:
        logger.error(f"Error in cloud_status: {e}")
        return jsonify({
            'provider': 'unknown',
            'status': 'error',
            'error': str(e),
...
def get_data():
    """Get data endpoint with proper error handling"""
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                'SELECT id, data, created_at FROM app_data ORDER BY created_at DESC LIMIT %s',
                (limit,)
            )
        ...
        items = [
            {
                'id': f"item_{i+1}",
                'name': f"Data Item {i+1}",
                'value': row.get('data', 'N/A'),
                'status': 'active',
        ...
        # Also include database status
        db_host = os.getenv('DB_HOST', '')
        db_type = 'RDS PostgreSQL' if '.rds.amazonaws.com' in db_host else 'Azure SQL'
...
def post_data():
    """Post data endpoint with validation"""
    ...
    try:
        ...
        logger.error(f"Database error in post_data: {e}")
        return jsonify({
            'error': 'Database error',
            'request_id': g.request_id
        }), 503
...
def main():
    ...
    logger.info("Initializing Service A...")
    ...
    logger.info(f"Starting Service A on {host}:{port} (debug={debug})")
